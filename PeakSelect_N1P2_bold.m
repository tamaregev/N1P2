function [peak_smpl, manual] = PeakSelect_N1P2_bold(wave, t, range, positive, sName, select_manual, select_largest, plotflag, onsetsamp)
% Tamar Oct 23 - adapted from PeakSelect_N1P2
...
%     xlabel('ms')
    ...
    plot(t,wave);
%     line([range(1) range(1)], [y(1) y(2)], 'Color', 'k');
%     line([range(2) range(2)], [y(1) y(2)], 'Color', 'k');
    if onsetsamp 
        line([t(onsetsamp) t(onsetsamp)], [y(1) y(2)], 'Color', 'r');
    end
...
%         line([range(2) range(2)], [y(1) y(2)], 'Color', 'k');
        if onsetsamp 
            line([0 0], [y(1) y(2)], 'Color', 'r');
        end
        plot(t(locs(local_pks)),pks(local_pks),'x','MarkerSize',10,'Color','r');
        for i = 1:length(local_pks)
            text(t(locs(local_pks(i))),pks(local_pks(i))+0.25,num2str(i));
        end
%         title(strcat(sName,' : ',txt));
%         pos = get(gcf,'position');
%         set(gcf,'position', [1 pos(2), pos(3), pos(4)]);
